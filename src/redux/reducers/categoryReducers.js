...
function CategoryReducer(state = initialState, action) {
  switch (action.type) {
    ...
    case SET_CATEGORY_ATTRIBUTES:
      return { ...state, categoryAttrebutes: action.payload.attributes }
...
export function getCategories() {
  return function (dispatch) {
    axios.get('https://ecommerce.main-gate.appx.uz/dev/v1/category/list')
...
export function getCategorySlug(slug) {
  return async function (dispatch) {
    ...
    Axios.get(GetCategoryWithSlug(slug)).then(function (response) {
      console.log(response.data)
      dispatch(SetCategoryProductsAC(response.data.products)) 
      dispatch(SetCategoryBrandsAC(response.data.categoryBrands))
      dispatch(SetCategoryAttributesAC(response.data.categoryAttrebutes))
